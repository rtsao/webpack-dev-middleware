...
module.exports = function Shared(context) {
	var share = {
		...
		setFs: function(compiler) {
			...
			function setNonNodeFs(c) {
				if (c.options.target === 'node' || c.outputFileSystem instanceof MemoryFileSystem) {
					// do nothing if target is node or fs is already memory
					return;
				}
				if (!fs) {
			...
			if (compiler.compilers) {
				compiler.compilers.forEach(setNonNodeFs);
				if (!fs) {
